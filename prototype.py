...
def draw_player():
    ...
    glTranslatef(13, 10, 75)     # Position similar to original
    glRotatef(-90, 1, 0, 0)    # Rotate to point forward
    glScalef(1.2, 1.2, 5)         # Shape it like a gun barrel
    glutSolidCube(5)
    glPopMatrix()
    ...
    # Gun
    # Gun (replaced cylinder with scaled cube)
    # Gun (scaled cube pointing forward)
    glPushMatrix()
    glColor3f(0.75, 0.75, 0.75)
    glTranslatef(0, 25, 75)     # Position similar to original
    glRotatef(-90, 1, 0, 0)    # Rotate to point forward
    glScalef(1.5, 1.5, 8)         # Shape it like a gun barrel
    glutSolidCube(5)
    glPopMatrix()
...
'''
    ...
    draw_wall(0, half_size + wall_thickness / 2, grid_row * tile_size, wall_thickness)
    glColor3f(0, 1, 0)  # Bottom wall
    draw_wall(0, -half_size - wall_thickness / 2, grid_row * tile_size, wall_thickness)
    glColor3f(0, 1, 1)  # Right wall
    draw_wall(half_size + wall_thickness / 2, 0, wall_thickness, grid_row * tile_size)
...
def move_enemy_towards_player():
    ...
    speed = 1  # Adjustment    for how  fast enemies move
...
def update_bullets():
    ...
    for bullet in bullets:
        ...
        # Check if the bullet is still within bounds
        if abs(bullet['pos'][0]) < GRID_WIDTH and abs(bullet['pos'][1]) < GRID_LENGTH:
            new_bullets.append(bullet)
        else:
            missed_bullets += 1
            print(" Missed bullet! Total missed:", missed_bullets)
    bullets = new_bullets
